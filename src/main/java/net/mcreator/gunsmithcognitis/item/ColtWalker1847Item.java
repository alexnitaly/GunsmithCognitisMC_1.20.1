...
public class ColtWalker1847Item extends Item {
	public ColtWalker1847Item() {
		super(new Item.Properties().durability(455).rarity(Rarity.COMMON));
	}
	...
	public InteractionResultHolder<ItemStack> use(Level world, Player entity, InteractionHand hand) {
		...
		if (ColtWalker1847CanUseRangedProcedure.execute(world, entity.getX(), entity.getY(), entity.getZ(), entity))
	...
	public void releaseUsing(ItemStack itemstack, Level world, LivingEntity entity, int time) {
		if (!world.isClientSide() && entity instanceof ServerPlayer player) {
			...
			if (player.getAbilities().instabuild || stack != ItemStack.EMPTY) {
				ColtWalker1847ProjectileEntity projectile = ColtWalker1847ProjectileEntity.shoot(world, entity, world.getRandom());
				...
				ColtWalker1847ItemUsedProcedure.execute(world, entity.getX(), entity.getY(), entity.getZ(), itemstack);
			}
		}
	}

	private ItemStack findAmmo(Player player) {
		ItemStack stack = ProjectileWeaponItem.getHeldProjectile(player, e -> e.getItem() == ColtWalker1847ProjectileEntity.PROJECTILE_ITEM.getItem());
		if (stack == ItemStack.EMPTY) {
			for (int i = 0; i < player.getInventory().items.size(); i++) {
				ItemStack teststack = player.getInventory().items.get(i);
				if (teststack != null && teststack.getItem() == ColtWalker1847ProjectileEntity.PROJECTILE_ITEM.getItem()) {
					stack = teststack;
					break;
				}
			}
